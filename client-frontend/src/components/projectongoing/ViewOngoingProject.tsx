...
export default function ViewOngoingProject() {
  const { id } = useParams();
    ...
    const fetchProject = async () => {
      try {
        ...
        const mapped = {
          ...
          freelancers: p.freelancers?.map((f: any) => ({
            id: f.freelancer.id,
  ...
  const handleOpenChat = (freelancer: any) => {
    navigate(`/projects/ongoing/${project?.id || "01"}/chat/${freelancer.id}`, {
...
`;
  ...
  const skills = project.skills?.length > 0 ? project.skills : ["React", "Tailwind CSS"];
  ...
  const freelancers = project.freelancers?.length > 0 ? project.freelancers : [
    {
      id: 1,
      name: "Rahul Sharma",
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-16">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs font-mono text-slate-400">ID: #{project.id || "08"}</span>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/90 shadow-sm p-7 space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">
            {project.name || "ewfewds"}
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            {project.postedDate?.includes("Posted") ? project.postedDate : `Posted ${project.postedDate || "02 Sep 2026"}`} •{" "}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 py-5 border-y border-slate-100">
          <div>
            ...
            <p className="text-lg font-serif font-bold text-slate-900 mt-1">{project.amount || "₹4,35,435"}</p>
          </div>

          <div>
            <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">COMPLETION DATE</p>
            <p className="text-lg font-medium text-slate-900 mt-1">{project.completionDate || "2026-09-09"}</p>
          </div>
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-slate-600 leading-relaxed">{project.description || "fsdfds"}</p>
        </div>
      ...
      {/* ========================================================================= */}
      {/* 2. BOTTOM CARD: FREELANCER APPROACHES WITH PROGRESS & PERCENTAGE          */}
      {/* ========================================================================= */}
      <div className="bg-white rounded-2xl border border-slate-200/90 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100">
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            {freelancers.length} freelancers have approaches on this project
          </p>
        </div>
                  ...
                  <td className="py-4 px-6">
                    <div className="flex items-center justify-center gap-2">
                      <button
                        ...
                        className="px-3.5 py-1.5 rounded-xl border border-slate-200 text-xs font-semibold text-slate-700 hover:bg-slate-50 hover:text-blue-600 transition shadow-xs cursor-pointer"
                      >
                        View Profile
                      </button>

                      {/* Talk Button Navigates to Chat */}
                      <button
                        onClick={() => handleOpenChat(f)}
                        className="px-5 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white text-xs font-semibold transition shadow-xs shadow-blue-500/20 cursor-pointer"
                      >
                        Talk
                      </button>
                    </div>
      ...
      {/* ========================================================================= */}
      {/* 3. FREELANCER MODAL (Only View & Close, No Chat)                          */}
      {/* ========================================================================= */}
